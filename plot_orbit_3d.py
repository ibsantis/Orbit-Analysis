...
for i in range(0, len(data['indices.z0'][:,0])):
    ...
    if data['infall.check'][i]:
        ...
        #
        prop = cm.jet((tlb-np.min(tlb))/(np.max(tlb)-np.min(tlb)))
        #
        ...
        sm = plt.cm.ScalarMappable(cmap=cm.jet, norm=plt.Normalize(vmin=tlb.min(), vmax=tlb.max()))
        plt.colorbar(sm, label='Lookback time [Gyr]')
        fig.suptitle('{0}: satellite {1}'.format(sim_data.galaxy, i+1))
        plt.tight_layout()
        plt.subplots_adjust(wspace=0.4, hspace=0)
        plt.savefig(sim_data.home_dir+'/orbit_data/plots/subhalo_integration/3d/'+sim_data.galaxy+'/'+sim_data.galaxy+'_sat_'+str(i+1)+'.pdf')
        plt.close()
