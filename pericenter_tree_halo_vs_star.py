...
"""
    ...
    ========================================
    = Pericenter tree halo vs star compare =
    ========================================
    ...
    in the tree based on the star mass.
...
if sim_data.num_gal == 1:
    ...
    host_positions = halt['star.position'][host_inds]
    host_velocities = halt['star.velocity'][host_inds]
    host_radii = halt['radius'][host_inds]
    #
    ...
    infall_info_star = orbits.infall_times(halo_pos_norm, snaps)
    #
    data_dict = dict()
    for i in peris.keys():
        data_dict[i+'.halt'] = peris[i]
        data_dict[i+'.star'] = peris_new[i]
    for i in infall_info.keys():
        data_dict[i+'.halt'] = infall_info[i]
        data_dict[i+'.star'] = infall_info_star[i]
    data_dict['dtot.halt'] = halt_dists
    data_dict['dtot.star'] = halo_pos
    #
    ut.io.file_hdf5(file_name_base=sim_data.home_dir+'/orbit_data/hdf5_files/peri_check/data_'+sim_data.galaxy+'_host_star_position', dict_or_array_to_write=data_dict, verbose=True)
...
if sim_data.num_gal == 2:
    ...
    data_dict['dtot.star'] = halo_pos
    #
    ut.io.file_hdf5(file_name_base=sim_data.home_dir+'/orbit_data/hdf5_files/peri_check/data_'+sim_data.gal_1+'_host_star_position', dict_or_array_to_write=data_dict, verbose=True)
    #
    ### GALAXY 2
    # Set up the halo inds and KDTree
    ...
    data_dict['dtot.star'] = halo_pos
    #
    ut.io.file_hdf5(file_name_base=sim_data.home_dir+'/orbit_data/hdf5_files/peri_check/data_'+sim_data.gal_2+'_host_star_position', dict_or_array_to_write=data_dict, verbose=True)
