...
for i in range(0, sub_pot.shape[0]):
    ...
    mask_sub = (np.flip(data['subhalo.pot'][i]) != -1)*np.isfinite(np.flip(data['subhalo.pot'][i]))*(data_total['m12i']['v.tot.sim'][i] != -1)*np.isfinite(data_total['m12i']['v.tot.sim'][i])*np.isfinite(np.flip(data['host.pot.100kpc']))
    #
    ...
    sub_energy[i][mask_sub*mask_host] = sub_pot[i][mask_sub*mask_host] + 0.5*(data_total['m12i']['v.tot.sim'][i][mask_sub*mask_host])**2


mask_low = (data_total['m12i']['infall.check'])*(data_total['m12i']['M.star.z0'] < 1e7)
mask_high = (data_total['m12i']['infall.check'])*(data_total['m12i']['M.star.z0'] > 1e7)
...
axs[0].add_patch(r1)
axs[1].add_patch(r2)
axs[0].text(1.2,2.25,'$M_{\\rm star} < 10^7 M_{\\odot}$',fontsize=20)
axs[1].text(1.2,2.25,'$M_{\\rm star} > 10^7 M_{\\odot}$',fontsize=20)
#
axs[0].set_xlim(0,13)
#axs[0].set_ylim(13,35)
axs[1].set_xlim(0,13)
#axs[1].set_ylim(14,33)
#
...
#plt.show()
plt.savefig('/Users/isaiahsantistevan/Desktop/E_vs_tlb.pdf')
